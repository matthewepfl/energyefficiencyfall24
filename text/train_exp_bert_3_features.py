...
# Load the data
listings_df = pd.read_csv('/scratch/izar/mmorvan/EnergyEfficiencyPrediction/data/Listings_FE.csv')
properties_test = pd.read_csv('/scratch/izar/mmorvan/EnergyEfficiencyPrediction/data/test_data_properties2.csv')['Property Reference Id'].unique().tolist()
...
expanded_keywords = pd.read_csv('/scratch/izar/mmorvan/EnergyEfficiencyPrediction/data/expanded_keywords.csv').iloc[:, 0].tolist()
...
# Fill NaN values in the additional variables with their respective means
for col in ['Price Extra Normalized', 'Size M2 Normalized', 'Year Built']:
    mean_value = text_train[col].mean()
    text_train[col].fillna(mean_value, inplace=True)
    text_test[col].fillna(mean_value, inplace=True)
...
class ListingsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        price_extra = self.data.iloc[index]['Price Extra Normalized']
        size_m2 = self.data.iloc[index]['Size M2 Normalized']
        year_built = self.data.iloc[index]['Year Built']
        ...
        return {
            ...
            'price_extra': torch.tensor(price_extra, dtype=torch.float),
            'size_m2': torch.tensor(size_m2, dtype=torch.float),
            'year_built': torch.tensor(year_built, dtype=torch.float),
...
class BERTRegressor(nn.Module):
    def __init__(self, bert_model, dropout_rate=0.3):
        ...
        self.regressor = nn.Sequential(
            nn.Linear(self.bert.config.hidden_size + 4, 128),  # +4 for additional variables
            nn.Dropout(dropout_rate),
            nn.Linear(128, 1)
        )

    def forward(self, input_ids, attention_mask, keyword_count, price_extra, size_m2, year_built):
        outputs = self.bert(input_ids=input_ids, attention_mask=attention_mask)
        pooled_output = outputs[1]
        combined_output = torch.cat(
            (pooled_output, keyword_count.unsqueeze(1), price_extra.unsqueeze(1), size_m2.unsqueeze(1), year_built.unsqueeze(1)),
            dim=1
        )
...
def train_epoch(model, data_loader, criterion, optimizer, scaler, device):
    ...
    for _, d in enumerate(tqdm(data_loader, desc="Training")):
        ...
        price_extra = d["price_extra"].to(device)
        size_m2 = d["size_m2"].to(device)
        year_built = d["year_built"].to(device)
        ...
        with torch.cuda.amp.autocast():
            outputs = model(input_ids=input_ids, attention_mask=attention_mask, keyword_count=keyword_count,
                            price_extra=price_extra, size_m2=size_m2, year_built=year_built)
